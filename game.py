...
class Asteroid:
    ...
    def spawn(self, camera_y: float, rocket_x: float) -> None:
        self.active = True
        self.size = random.uniform(ASTEROID_MIN_SIZE, ASTEROID_MAX_SIZE)
        target_x = rocket_x + ROCKET_W / 2 + random.uniform(-170, 170)
        self.x = max(50, min(SCREEN_WIDTH - 50, target_x))
        # Static world obstacle placed ahead of the rocket path.
        self.y = camera_y - random.uniform(SCREEN_HEIGHT * 0.65, SCREEN_HEIGHT * 1.35)
        ...
        for i in range(n):
            a = (i / n) * math.tau
            r = random.uniform(self.size * 0.7, self.size * 1.3)
...
class RocketGame:
    ...
    def update_asteroids(self) -> None:
        ...
        while self.active_asteroids < ASTEROID_TARGET_ACTIVE:
        ...
        rocket_rect = pygame.Rect(int(self.rocket_x), int(self.rocket_y), ROCKET_W, ROCKET_H + ROCKET_NOSE_H)
        ...
        for a in self.asteroids:
            ...
            asteroid_rect = pygame.Rect(int(a.x - a.size), int(a.y - a.size), int(a.size * 2), int(a.size * 2))
            if rects_overlap(rocket_rect, asteroid_rect):
                if self.shield_active:
                    a.active = False
                    self.active_asteroids = max(0, self.active_asteroids - 1)
                    self.score += 5
    ...
    def update_pickups(self) -> None:
        ...
        for p in self.pickups:
            ...
            p_rect = pygame.Rect(int(p.x - p.size), int(p.y - p.size), p.size * 2, p.size * 2)
            if rects_overlap(rocket_rect, p_rect):
                if p.kind == "fuel":
    ...
    def draw_background(self) -> None:
        ...
        sky_t = min(1.0, altitude / 8000)
        for y in range(SCREEN_HEIGHT):
            blend = y / SCREEN_HEIGHT
            r = int((8 + blend * 30) * (1 - sky_t) + 2 * sky_t)
            g = int((20 + blend * 70) * (1 - sky_t) + 4 * sky_t)
            b = int((60 + blend * 130) * (1 - sky_t) + 15 * sky_t)
    ...
    def draw_rocket(self) -> None:
        ...
        if self.stage == 1:
            pygame.draw.rect(self.screen, GRAY, (x - booster_w, booster_y, booster_w, ROCKET_THRUSTER_H))
            pygame.draw.rect(self.screen, GRAY, (x + ROCKET_W, booster_y, booster_w, ROCKET_THRUSTER_H))
            pygame.draw.rect(self.screen, RED, (x - booster_w, booster_y + ROCKET_THRUSTER_H // 2, booster_w, band_h))
            pygame.draw.rect(self.screen, RED, (x + ROCKET_W, booster_y + ROCKET_THRUSTER_H // 2, booster_w, band_h))

        pygame.draw.rect(self.screen, LIGHT_GRAY, (x, y, ROCKET_W, ROCKET_H))
        pygame.draw.rect(self.screen, RED, (x, int(y + ROCKET_H * 0.25), ROCKET_W, band_h))
        pygame.draw.rect(self.screen, RED, (x, int(y + ROCKET_H * 0.55), ROCKET_W, band_h))

        window_cy = int(y + ROCKET_H * 0.40)
        pygame.draw.circle(self.screen, BLUE, (x + ROCKET_W // 2, window_cy), win_r)
        pygame.draw.circle(self.screen, WHITE, (x + ROCKET_W // 2, window_cy), win_r, 2)

        nose = [(x, y), (x + ROCKET_W // 2, y - ROCKET_NOSE_H), (x + ROCKET_W, y)]
        pygame.draw.polygon(self.screen, WHITE, nose)
        pygame.draw.polygon(self.screen, GRAY, nose, 1)

        pygame.draw.polygon(self.screen, GRAY, [(x, y + ROCKET_H - fin_h), (x - fin_w, y + ROCKET_H), (x, y + ROCKET_H)])
        pygame.draw.polygon(
            self.screen,
            GRAY,
            [(x + ROCKET_W, y + ROCKET_H - fin_h), (x + ROCKET_W + fin_w, y + ROCKET_H), (x + ROCKET_W, y + ROCKET_H)],
        )

    def draw_shield(self) -> None:
        cx = int(self.rocket_x + ROCKET_W / 2)
        cy = int(world_to_screen_y(self.rocket_y, self.camera_y) + ROCKET_H / 2)
        radius = 88 + int(math.sin(self.frame * 0.15) * 8)
