...
PICKUP_ICON_URLS = {
    "fuel": "https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/26fd.png",
    "shield": "https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/1f6e1.png",
    "clear": "https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/1f4a3.png",
}
PICKUP_ICON_DIR = "assets/pickups"
...
class Asteroid:
    ...
    def spawn(self, camera_y: float, rocket_x: float, boss: bool = False) -> None:
        self.active = True
        self.is_boss = boss
        self.depth = random.uniform(0.82, 1.22)
        self.close_call = False
    ...
    def draw(self, screen: pygame.Surface, camera_y: float) -> None:
        ...
        # Pseudo-3D perspective: farther (upper) objects render smaller.
        perspective = max(0.62, min(1.38, (0.74 + (sy / SCREEN_HEIGHT) * 0.84) * self.depth))
        pts = []
        for px, py in self.points:
            rx = (px * c - py * s) * perspective
            ry = (px * s + py * c) * perspective
...
class Pickup:
    ...
    def draw(
        ...
        perspective = max(0.70, min(1.35, 0.78 + (sy / SCREEN_HEIGHT) * 0.82))
        pulsed = max(8, int((self.size + math.sin(self.pulse) * 3) * perspective))
        ...
        if icon is not None:
            sz = max(12, int(pulsed * 2.1))
            scaled = pygame.transform.smoothscale(icon, (sz, sz))
            screen.blit(scaled, (int(self.x - sz // 2), int(sy - sz // 2)))
...
class RocketGame:
    ...
    def _prepare_pickup_images(self) -> None:
        icon_dir = Path(PICKUP_ICON_DIR)
        try:
            icon_dir.mkdir(parents=True, exist_ok=True)
        except OSError:
            return
        for kind, url in PICKUP_ICON_URLS.items():
            icon_path = icon_dir / f"{kind}.png"
            if not icon_path.exists():
                try:
                    ...
                    continue
            try:
                self.pickup_images[kind] = pygame.image.load(str(icon_path)).convert_alpha()
            except pygame.error:
                continue
    ...
    def update_pickups(self) -> None:
        if self.obstacles_since_powerup >= self.next_pickup_dodge_target:
            ...
            if slot:
                ...
                for _ in range(5):
                    slot.spawn(self.camera_y, self.rocket_x)
                    too_close = any(
                        a.active and math.hypot(slot.x - a.x, slot.y - a.y) < (a.size + 85)
                        for a in self.asteroids
                    )
                    if not too_close:
                        spawned = True
        ...
        for p in self.pickups:
            ...
            # Soft magnet helps avoid "I touched it but missed by 1px" frustration.
            if dist < (p.size + ROCKET_W * 0.75):
                p.x += (rocket_cx - p.x) * 0.035
                p.y += (rocket_cy - p.y) * 0.035
    ...
    def draw_pickups(self) -> None:
        for p in self.pickups:
            p.draw(self.screen, self.camera_y, self.font_sm, self.pickup_images.get(p.kind))
